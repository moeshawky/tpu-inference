...
from tpu_inference.layers.vllm.quantization.unquantized import (
    ...
    VllmUnquantizedLinearMethod, _host_numpy_from_torch,
...
def test_load_weight_for_layer_non_pathways(dtype):
    """_load_weight_for_layer shards on host and places onto the requested sharding."""
    ...
    # Placed, not replicated on the default device, and fully addressable so no
    # second reshard (shard_sharded_device_array_slow_path / VFIO 4GiB class) is needed.
...
# --- _host_numpy_from_torch / _release_cpu_storage unit tests ---


def test_host_numpy_from_torch_bf16_bit_exact():
    """_host_numpy_from_torch bit-casts bf16 through uint8 with no upcast."""
    import ml_dtypes

    import numpy as np

    layer = _make_layer_with_weight((4, 8), torch.bfloat16)
    layer.weight = torch.nn.Parameter(
        torch.tensor([[1.5, 2.25, -3.5, 4.0] * 2] * 4, dtype=torch.bfloat16))
    np_result = _host_numpy_from_torch(layer.weight)
    assert np_result.dtype == ml_dtypes.bfloat16
    expected = layer.weight.detach().cpu().view(torch.uint8).numpy().view(
        ml_dtypes.bfloat16)
    np.testing.assert_array_equal(np_result, expected)


def test_host_numpy_from_torch_fp32_passthrough():
    """_host_numpy_from_torch returns plain float32 numpy for fp32 input."""
    import numpy as np

    layer = _make_layer_with_weight((2, 3), torch.float32)
    np_result = _host_numpy_from_torch(layer.weight)
    assert np_result.dtype == np.float32
    assert np_result.shape == (2, 3)


def test_release_cpu_storage_swallows_non_resizable():
    """_release_cpu_storage tolerates safetensors-style non-resizable storage."""
    ...
    tensor = MagicMock()
    tensor.untyped_storage.return_value = _NonResizable()
    # Must not raise.
    _release_cpu_storage(tensor)


def test_release_cpu_storage_reraise_other_errors():
    """_release_cpu_storage re-raises RuntimeError values that are not the narrow non-resizable case."""
    ...
    tensor = MagicMock()
    tensor.untyped_storage.return_value = _Boom()
