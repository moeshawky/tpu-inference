...
def physical_page_size(mode: Mode, kv_cache_block_size: int,
    ...
    """
    # Compressed KV tokens per page (vLLM's ``spec.block_size`` / ``spec.num_states`` — was ``spec.storage_block_size``).
    block_size = kv_cache_block_size // compress_ratio
    if mode is Mode.CSA_INDEXER:
        # (N, T // 4, 4, 256) u8
        return block_size // SLOT_PACK
    if mode is Mode.CSA:
        # (N, T, 4, 128) u8
        return block_size
    # HCA (N, T * 2, 4, 128) u8
