...
class VllmQKVParallelLinear(QKVParallelLinear):
    ...
    def forward(
        ...
        # NOTE (2026-09-10): Skip the KV-replica shard_map annotation.
        # It previously built a 3-axis (data, model, replica) mesh nested
        # inside the ambient 2-axis (data, model) mesh, which JAX rejects:
        #   ValueError: context mesh ('data':1,'model':2,'replica':4) should
        #   match shard_map mesh ('data':1,'model':8).
        # The annotated op was value-identity (`return t`), so skipping it
        # cannot change values; downstream sees standard ambient sharding.
        # KV-head replication is already handled by _tile_kv at weight-load
        # time; the replica sub-axis is not needed at runtime.
